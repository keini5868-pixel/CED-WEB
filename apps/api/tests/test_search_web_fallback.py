...
def test_search_web_timeout_returns_fallback():
    def slow_brief(*_args, **_kwargs):
        time.sleep(SEARCH_WEB_TIMEOUT_SEC + 2)
        return {"ok": True, "summary": "resultado tardío"}

    with patch("app.services.voice_tool_executor.fetch_voice_brief", slow_brief):
        result = asyncio.run(
            execute_voice_tool(
                "search_web",
                "user-test-123",
...
def test_search_web_empty_result_returns_fallback():
    with patch(
        "app.services.voice_tool_executor.fetch_voice_brief",
        lambda *_a, **_k: {"ok": False, "error": "vacío", "code": "empty_result"},
    ):
        result = asyncio.run(
            execute_voice_tool(
                "search_web",
