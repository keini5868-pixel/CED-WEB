...
export function AdvancedChatPanel({
  ...
  const panel = (
      ...
      >
        ...
        <footer className={`relative z-20 shrink-0 overflow-visible px-3 pt-2 pb-2 sm:px-4 sm:pt-3 border-t border-[var(--studio-border)] bg-[var(--studio-chat-bg)] ${embedded ? "lg:pb-3" : "pb-[max(0.75rem,env(safe-area-inset-bottom))]"}`}>
          {attachedPdf ? (
            <PdfAttachmentBar
