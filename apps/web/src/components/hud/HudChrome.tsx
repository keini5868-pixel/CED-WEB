...
export function HudChrome({ email, isSuperAdmin }: HudChromeProps) {
  ...
  return (
    <header className="sticky top-0 z-50 w-full max-w-[100vw] overflow-visible border-b border-cyan-500/20 bg-[var(--ced-bg)]/95 backdrop-blur-sm">
      <div className="mx-auto flex w-full max-w-screen items-center gap-2 px-3 py-2 sm:px-4 sm:py-2.5 md:px-6">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <button
            ...
            className="inline-flex rounded border border-cyan-500/30 p-1.5 text-cyan-400 hover:border-cyan-400/60 hover:text-cyan-200"
          >
            <Navigation className="h-3.5 w-3.5" strokeWidth={2} />
          </button>
        </div>

        <div className="hidden min-w-0 flex-[2] justify-center px-3 md:flex">
          <BibleVerseTicker />
        </div>
      ...
      <div className="border-t border-cyan-500/10 px-3 py-1 md:hidden">
        <BibleVerseTicker />
      </div>
    </header>
  );
